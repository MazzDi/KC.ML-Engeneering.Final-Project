...
async def get_manager(user_id: int, session: Session = Depends(get_session)) -> ManagerRead:
...
async def get_manager_clients(user_id: int, session: Session = Depends(get_session)) -> list[ClientRead]:
