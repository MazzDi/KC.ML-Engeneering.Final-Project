...
### Глоссарий и формулы
- Net Interest Income (NII): $\mathrm{NII}=\text{Процентные доходы}-\text{Процентные расходы}$.
- Cost of Risk (CoR): $\mathrm{CoR}=\frac{\text{Расходы на резервы}}{\text{Средний кредитный портфель}}$.
- Approval Rate: $\mathrm{Approval\ Rate}=\frac{\text{Одобренные заявки}}{\text{Все заявки}}$.
